    ...
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute top-full mt-1 left-0 bg-white border border-gray-100 rounded-xl shadow-2xl z-50 min-w-[210px] py-1 overflow-visible">
          <div className="px-4 py-2.5 text-sm text-gray-600 hover:bg-blue-50 hover:text-blue-600 cursor-pointer transition-colors"
        ...
        <div className="relative">
          ...
          <button onClick={() => scroll(-1)} disabled={!canScrollLeft}
            className={`absolute left-0 top-1/2 z-20 w-14 h-14 rounded-full bg-white shadow-xl flex items-center justify-center transition-all duration-200
              ${canScrollLeft ? "text-purple-700 hover:bg-purple-100 hover:scale-105 opacity-100" : "text-gray-300 opacity-40 cursor-not-allowed"}`}
            style={{ transform: "translate(-30%, -50%)" }}>
            <ChevronLeft className="w-7 h-7 stroke-[2.5]" />
          </button>
          <button onClick={() => scroll(1)} disabled={!canScrollRight}
            className={`absolute right-0 top-1/2 z-20 w-14 h-14 rounded-full bg-white shadow-xl flex items-center justify-center transition-all duration-200
              ${canScrollRight ? "text-purple-700 hover:bg-purple-100 hover:scale-105 opacity-100" : "text-gray-300 opacity-40 cursor-not-allowed"}`}
            style={{ transform: "translate(30%, -50%)" }}>
            <ChevronRight className="w-7 h-7 stroke-[2.5]" />
          </button>
          <div ref={scrollRef} className="flex gap-6 overflow-x-auto pb-3" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
            {resources.slice(0, 12).map((resource) => (
              <div key={toStr(resource._id)} className="flex-shrink-0" style={{ width: 320 }}>
                <RecommendationCard resource={resource} toast={toast} />
              </div>
            ))}
          </div>
        </div>
...
export default function OrganizerPage() {
  ...
  const handleSendClick = () => {
    ...
    toast.success(`${cartItems.length} demande(s) prêtes à être envoyées`, "Redirection...", 2000);
    setTimeout(() => navigate("/mes-reservations"), 600);
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const r = await fetch("http://localhost:5000/api/recommendations", {
        ...
        toast.warning("Recommandations temporairement indisponibles", "Avertissement");
      } finally {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const r = await fetch("http://localhost:5000/api/recommendations/event", {
          method: "POST",
        ...
        setRecommendedResources([]);
        toast.error("Impossible de charger les recommandations pour cet événement", "Erreur");
      } finally {
        setLoadingRecs(false);
      }
    })();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50">
      ...
      <div className="pt-28 pb-10 px-4 max-w-7xl mx-auto">
        ...
        {/* Carousel */}
        <RecommendationsCarousel toast={toast} resources={recommendedResources} loading={loadingRecs} title={getRecsTitle()} />
                ...
                {currentResources.map((resource) => (
                  <ResourceCard
                    ...
                    onBook={() => {
                      ...
                      toast.info(`Ouverture de la réservation pour "${resource.name}"`, "Réservation", 2000);
                    }}
                  />
                ))}
              </div>
