...
class ApiService {
  ...
  static Future<Map<String, dynamic>?> getResource(String id) async {
    try {
      final res = await http.get(
        Uri.parse('$kBaseUrl/ressources/get_by_id/$id'),
      );
  ...
  static Future<List<Reservation>> getMyReservations() async {
    ...
    final res = await http.get(
      Uri.parse('$kBaseUrl/location/get_my_locations'),
      headers: headers,
    );
  ...
  static Future<List<EventModel>> getUserEvents() async {
    ...
    if (uid == null) return [];
    final res = await http.get(
      Uri.parse('$kBaseUrl/event/user_event/$uid'),
      headers: headers,
    );
    if (res.statusCode == 200) {
      final List data = jsonDecode(res.body);
  ...
  static Future<String?> createEvent(
    ...
    final res = await http.post(
      Uri.parse('$kBaseUrl/event/create'),
      headers: {
        'Content-Type': 'application/json',
...
class _MesReservationsPageState extends State<MesReservationsPage> {
  ...
  Future<void> _sendRequest(CartItem item, String eventId) async {
    ...
    _toast(
      allOk
          ? 'Demande envoyée pour "${item.resourceName}" !'
  ...
  Widget build(BuildContext context) {
    if (_loading) {
      return const Scaffold(
        backgroundColor: kBg,
        body: Center(child: CircularProgressIndicator(color: kIndigo)),
      );
    }
              ...
              child: RefreshIndicator(
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    if (_token == null && _cart.isNotEmpty) ...[
                      _buildLockBanner(),
                      const SizedBox(height: 14),
                    ],
                    if (_cart.isEmpty && _reservations.isEmpty)
                      ...
                      if (_cart.isNotEmpty) ...[
                        _buildSectionHeader(
                          ...
                          '$_totalCart article${_totalCart > 1 ? 's' : ''}',
          ...
          const Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 2),
                Text(
                  'Cliquez "Envoyer" sur un article pour vous connecter.',
            ...
            children: [
              ...
              Text(
                ...
                style: TextStyle(
                  fontSize: 11,
                  fontWeight: FontWeight.w600,
                  color: res.isPaid ? kPurple : kIndigo,
                ),
              ),
